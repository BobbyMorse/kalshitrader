...
def find_ladder_mean_reversion(
    ...
    for event_ticker, markets in groups.items():
        ...
        for k in range(1, len(sorted_markets) - 1):
            ...
            # Max price gap between neighbors: if they differ by >35¢, the middle sits
            # at a steep curve where interpolation is unreliable (e.g. 93¢/5¢ neighbors).
            ...
            if lower_nb.yes_bid < 0.05 or upper_nb.yes_bid < 0.05:
                ...
                continue
            if abs(lower_nb.mid() - upper_nb.mid()) > 0.35:
                _dbg_inc(series, "neighbor_gap_too_large")
                continue
            if nb_spread_lower > 0.20 or nb_spread_upper > 0.20:
